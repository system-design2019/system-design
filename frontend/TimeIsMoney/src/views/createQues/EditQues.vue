        ...
        <div style="overflow:hidden">
            ...
            <div style="float: right; width: 90%">
                <div style="float: left; width: 70%; position: relative; border: 2px #000000;">
                    ...
                    <div>
                        <Card v-for="(q, index) in questions" :key="index" style="margin: 5px 0" @click.native="curr=index">
                            ...
                            <div v-else>
                                <Checkbox v-for="(c,index) in q.choices" style="width: 100%; margin: 5px 0">{{c}}</Checkbox>
                            </div>
                        </Card>
                    </div>
                </div>
                <div style="float: left; width: 30%; position: relative; padding: 0 1%">
                    <Card>
                        <p>类型：{{getMode(questions[curr].mode)}}</p>
                        <Divider />
                        <p>标题</p><Input v-model="questions[curr].title"></Input>
                        <Divider />
                        <div v-if="questions[curr].mode === 2">
                            ...
                            <div style="margin-top: 10px">
                                <span>最多可选：</span>
                                <InputNumber :max="questions[curr].choices.length" :min="1" v-model="questions[curr].choose"></InputNumber>
                            </div>
                            <Divider />
                        </div>
                        <div>
                            <span>修改顺序：</span>
                            <InputNumber :max="questions.length" :min="1" v-model="pos"></InputNumber>
                            <Button @click="changeOrder()">确定</Button>
                            <Divider />
                        </div>
                        <div>
                            <span>必填：</span>
                            <Switch v-model="questions[curr].fill" />
                            <Divider />
                        </div>
                        <div>
